...
- **部署无法服务的路由拥有规范 code。** 无法解析到凭据的路由公开 `MISSING_CREDENTIAL`，账户无权使用的模型公开 `MODEL_NOT_ENTITLED`，无论从哪个检测点到达——适配器自身的凭据解析，或压平进带内 finish 错误的提供方措辞。两者对该路由都是确定性的，因此都不可重试，而 `dsh-tool-subagent` 会据此让被委派的子智能体符合其回退路由的条件。
