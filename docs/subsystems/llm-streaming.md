...
- **A route the deployment cannot serve has canonical codes.** A route with no resolvable credential surfaces `MISSING_CREDENTIAL` and a model the account is not permitted to use surfaces `MODEL_NOT_ENTITLED`, whichever detection point reaches them — the adapter's own credential resolution, or provider wording flattened into an in-band finish error. Both are deterministic for that route, so neither is retryable, and `dsh-tool-subagent` qualifies a delegated child for its fallback route on them.
