...
- **两种路由拒绝依赖提供方措辞识别**：pi-ai 把捕获到的错误压平成它的 message，因此 `Provider is not configured: <route>`（`MISSING_CREDENTIAL`）与模型准入拒绝（`MODEL_NOT_ENTITLED`）都是在所有状态码数字测试之前按文本匹配的。若 pi-ai 或提供方的某个版本改写了其中任一措辞，它会退回到兜底的 `PI_AI_ERROR`——仍可诊断，但不再能让被委派的子智能体符合回退路由的条件。
