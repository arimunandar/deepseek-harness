...
- **Two route refusals are recognized by provider wording** — pi-ai flattens the caught error to its message, so `Provider is not configured: <route>` (`MISSING_CREDENTIAL`) and a model-entitlement refusal (`MODEL_NOT_ENTITLED`) are matched as text ahead of every status-digit test. A pi-ai or provider release that rewords either one returns it to the catch-all `PI_AI_ERROR`, which is diagnosable but no longer qualifies a delegated child for a fallback route.
