...
Shared subprocess harness for tests that boot an app and `cordis.yml` through the Cordis Loader. `resolveExampleLaunch` selects local `src` mode (tsx and root tsconfig paths) or CI `lib` mode (plain Node and package exports) from an explicit mode or `DSH_EXAMPLE_MODE`. Either mode adds `--disable-warning=ExperimentalWarning` to the child's `NODE_OPTIONS`, keeping whatever the caller or the environment already set, so Node's own warning for an experimental built-in never reaches a test asserting clean stderr; every other warning still does.
