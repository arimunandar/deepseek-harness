...
用于测试通过 Cordis Loader 启动应用和 `cordis.yml` 的共享子进程 harness。`resolveExampleLaunch` 选择本地 `src` mode（tsx 和根 tsconfig 路径）或 CI `lib` mode（普通 Node 和包导出）；选择依据为显式 mode 或 `DSH_EXAMPLE_MODE`。两种 mode 都会向子进程的 `NODE_OPTIONS` 追加 `--disable-warning=ExperimentalWarning`，并保留调用方或环境已设置的值，因此 Node 针对实验性内置模块发出的警告不会到达断言 stderr 为空的测试；其余警告仍会照常出现。
