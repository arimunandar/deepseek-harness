...
**把首次运行留给官方 DeepSeek 步骤。** 那是本功能最初的姿态，在此记录为它现在所取代的备选方案。它先是带着接管界面做出来，因为两个会对推迟第一个的人叠加而在落地前被移除，又在卡片能接受手输密钥之后被恢复——正是这一点让更窄的那个步骤得以退场而不是共存。代价是目录只知道它所承载的后端：如果某人唯一可用的提供方是 `Config.connections` 之外的一条路由，他仍然会被询问，而旧步骤对任何可达的提供方都会结束。推迟只需一次点击，背后也没有任何东西被阻塞。

最初的推理保留于此，因为正是它让这个排序变得安全：**它自己的首次运行接管界面。** 做出来了，落地前又移除了。`settings.onboarding` 已经有占用者——官方 DeepSeek 凭据步骤——把它注册在前面，会让推迟了第一个的人把首次运行走成两层接管；现有的 `onboarding-deepseek-config` 与 `onboarding-usable-provider` 场景立刻抓到了这一点：这个步骤抢在凭据对话框前面，并且在其中一个场景断言"不绘制接管界面"的那次重载里，把应用根节点保持为 inert。那些失败是设计问题，不是测试问题：哪个步骤拥有首次运行，是关于那条流程的决定，而把它折进一个页面，正是首次运行被意外重新设计的方式。在那个决定被慎重做出之前，这个页面通过设置页抵达；代价被记录为限制而不是被藏起来——没有凭据的人依然遇到的是更窄的 DeepSeek 问题，要走到 Claude 或 Codex 得先找到设置页。
...
首次运行现在就是这个页面：用产品术语列出三个后端、每个一个按钮，而以手输密钥抵达的后端配一个遮蔽输入框。人在那里读到的词汇从不包含凭据、引用、记录、路由、提供方或命名空间。`ui-settings-models` 不再注册它的凭据步骤；该组件及其就绪投影仍然导出，供想要那个更窄步骤的组合使用。
