...
Settings now has a page that names three backends in product terms with one button each, and the vocabulary a person reads there never includes credential, reference, record, route, provider, or namespace. First run still belongs to the narrower DeepSeek step, so the page is something they find rather than something they meet.
